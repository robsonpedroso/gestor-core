...
namespace Gestor.Tools.WebApi
{
    public static class ServiceCollectionExtensions
    {
        ...
        public static void AddServiceMappingsFromAssemblies<TApplication, TDomain, TInfra>(
            ...
            where TInfra : class
        {
            var iUnitOfWorkType = typeof(IUnitOfWork);
            ...
            var uow = infraAssembly.DefinedTypes.FirstOrDefault(t => !t.IsAbstract && (t.ImplementedInterfaces?.Any(i => i.FullName == iUnitOfWorkType.FullName) ?? false));

            if (!uow.IsNull())
                serviceCollection.AddScoped(iUnitOfWorkType, uow.UnderlyingSystemType);
